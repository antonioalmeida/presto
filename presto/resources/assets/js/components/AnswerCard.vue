	...
	<router-link :to="'/questions/' + answer.question.id + '/answers/' + answer.id" class="list-group-item list-group-item-action flex-column align-items-start">
		<div class="d-flex w-100 justify-content-between flex-column mb-1">
			...
			<div class="d-flex">
				...
				<h6>
					<router-link :to="'/profile/' + answer.author.username" class="btn-link">{{answer.author.name}}</router-link><br>
		...
		<small class="text-muted"><i class="far fa-tags"></i>
				<router-link v-for="(topic, index) in answer.question.topics" class="text-muted" :key="topic.id" :to="'/topic/' + topic.name">
	                {{ topic.name }}<template v-if="index != answer.question.topics.length -1">,
</template>
            </router-link>
