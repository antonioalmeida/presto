...
<div onclick="location.assign('/questions/{{$answer->question->id}}/answers/{{$answer->id}}');" class="list-group-item list-group-item-action flex-column align-items-start">
                                        ...
                                        <p class="mb-1">{!!substr(strip_tags($answer->content), 0, 169)!!}{{(strlen(strip_tags($answer->content))>169 ? '...' : '')}}<span href="/questions/{{$answer->question->id}}/answers/{{$answer->id}}" class="btn-link text-primary">(read more)</span></p>
