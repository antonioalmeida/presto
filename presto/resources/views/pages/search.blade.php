            ...
            <div class="row">
                    ...
                    <div class="offcanvas-collapse" id="navbarsExampleDefault">
                                ...
                                @foreach($filter_types as $type_filter)
                                    <div class="form-check">
                                        <form method="POST" action="{{ Route('search')}}">
                                            ...
                                            @if($type === $type_filter)
                                                <input type="submit" value="<?=ucfirst($type_filter)?>"
                                                       class="filter-btn btn-link selected-filter"/>
                                            @else
                                                <input type="submit" value="<?=ucfirst($type_filter)?>"
                                                       class="filter-btn btn-link"/>
                                            @endif
                                        </form>
                                    </div>
                                @endforeach
                        ...
                        <div>
                            ...
                            <div class="input-group">
                                ...
                                @else
                                    <input type="text" class="form-control filter-disabled" placeholder="Find People"
                                           disabled>
                                @endif
                            </div>
                        </div>
                                ...
                                @foreach($filter_dates as $date_filter)
                                    <div class="form-check">
                                        <form method="POST" action="{{ Route('search')}}">
                                            ...
                                            <input type="hidden" name="limit_date"
                                                   value="<?=date('Y-m-d H:i:s', strtotime($date_filter[0]))?>"/>
                                            @if($type == 'questions' || $type == 'answers')
                                                @if(date('Y-m-d', strtotime($limit_date)) === date('Y-m-d', strtotime($date_filter[0])))
                                                    <input type="submit" value="<?=$date_filter[1]?>"
                                                           class="btn-link text-muted filter-btn selected-filter"/>
                                                @else
                                                    <input type="submit" value="<?=$date_filter[1]?>"
                                                           class="btn-link text-muted filter-btn"/>
                                                @endif
                                            @else
                                                <input type="submit" value="<?=$date_filter[1]?>"
                                                       class="btn-link text-muted filter-btn filter-disabled" disabled/>
                                            @endif
                                        </form>
                                    </div>
                                @endforeach
                ...
                <div class="col-md-8">
                    <div class="d-flex justify-content-between flex-wrap">
                        <h3>
                            <small class="text-muted">Results for</small> {{$query}}</h3>
                    ...
                    @if($type == 'questions' || $type == 'answers')
                        <nav>
                            <div class="nav nav-tabs" id="nav-tab" role="tablist">
                                <a class="nav-item nav-link active" id="nav-newest-tab" data-toggle="tab"
                                   href="#nav-newest" role="tab" aria-controls="nav-newest"
                                ...
                                <div class="list-group">
                                    @switch($type)
                                        ...
                                        @foreach($result as $member)
                                            <div onclick="location.assign('{{Route('profile', $member->username)}}');"
                                        ...
                                        @foreach($result as $topic)
                                            <div onclick="location.assign('{{Route('topic', $topic->name)}}');"
                                                 class="list-group-item list-group-item-action flex-column align-items-start">
                                                <div class="d-flex w-100 justify-content-begin">
                                                    <div class="align-self-center">
                                                        <img class="rounded-circle pr-2" width="60px" heigth="60px"
